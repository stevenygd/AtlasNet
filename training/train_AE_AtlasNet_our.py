...
dataset = ShapeNet15kPointClouds(split='train', categories=opt.class_choice)
...
for epoch in range(opt.nepoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader_test, 0):
            ...
            points = points.cuda()
            #SUPER_RESOLUTION
            assert points.size(2) >= 2 * opt.super_points
            input_points = points[:,:,:opt.super_points].contiguous()
            test_points = points[:,:,opt.super_points:2*opt.super_points].contiguous()
            #END SUPER RESOLUTION
            pointsReconstructed  = network(input_points)
            ...
            if i%200 ==0 :
                vis.scatter(X = input_points.transpose(2,1).contiguous()[0].data.cpu(),
                        win = 'VAL_INPUT',
                        opts = dict(
                            title = "VAL_INPUT",
                            markersize = 2,
