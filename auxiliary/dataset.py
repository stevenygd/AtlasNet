...
class ShapeNet(data.Dataset):
    def __init__(self,
            catefilename='./data/synsetoffset2category.txt',
        ...
        self.datapath = []
        self.catfile = os.path.join(catefilename)
        self.cat = {}
        self.meta = {}
        self.SVR = SVR
        ...
        with open(self.catfile, 'r') as f:
            for line in f:
                ls = line.strip().split()
                self.cat[ls[0]] = ls[1]
        if not (class_choice is None or "all" in class_choice):
        ...
        for item in self.cat:
            ...
            try:
                ...
                fns_pc = []
            fns = [val for val in fns_img if val + '.npy' in fns_pc]
        ...
        self.validating = transforms.Compose([transforms.CenterCrop(127),])
    ...
    def __getitem__(self, index):
        ...
        idx = np.random.choice(np.arange(point_set.shape[0]), self.npoints)
        point_set = point_set[idx,:]
        assert point_set.shape[0] == self.npoints, \
                "PointSet shape[0] = %d Npoints = %d"%(point_set.shape[0], self.npoints)
